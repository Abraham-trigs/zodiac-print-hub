 ...
 * Updated to sync with the isolated pricingDraft bucket.
 */
export function MaterialServiceCatalog() {
  ...
  // ✅ Switch from setDraft to setPricingDraft
  ...
  const handleSelect = (item: any) => {
    // ✅ Commit data to the Pricing Workstation Draft bucket
    ...
    <div className="flex flex-col h-full gap-6 animate-in fade-in zoom-in-95 duration-500 bg-black p-6">
          ...
          <div className="flex gap-2 mt-1">
            {["ALL", "MATERIAL", "SERVICE"].map((m) => (
              <button
                ...
                className={`text-[8px] font-black px-2 py-1 rounded transition-all ${
                  filterMode === m
      ...
      <div className="relative">
        <input
          ...
          className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-12 text-sm focus:border-cyan-400 outline-none transition-all placeholder:opacity-10 text-white font-bold"
        />
          ...
          >
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                <span
                  className={`text-[10px] font-black px-2 py-0.5 rounded border border-white/10 uppercase tracking-tighter ${
                    item.type === "MATERIAL"

import { Job, Material } from "@prisma/client";

interface NestedJob extends Job {
  x: number;
  y: number;
}
...
export function resolveLayout(jobs: Job[], material: Material) {
  ...
  const nestedItems = jobs.map((job) => {
    const jobWidthWithBleed = job.width + safetyBleed * 2;
    const jobHeightWithBleed = job.height + safetyBleed * 2;
    ...
    totalJobArea += job.width * job.height;
  ...
  return {
    ...
    wasteArea: Math.max(0, totalCapacityArea - totalJobArea),
  };
}
